...
// ROS includes
#include <terpcopter_comm/Mavlink.h>
...
#include "terpcopter_driver/mavlink_serial_comm.h"
...
SerialComm::open(const std::string& portStr, int baudrate)
{
    ...
    // set up publishers
    m_mavlinkPub = m_nh.advertise<terpcopter_comm::Mavlink>("mavlink", 100);
    m_optFlowPub = m_nh.advertise<terpcopter_comm::OpticalFlow>("opt_flow", 5);
    ...
    for (size_t i = 0; i < bytesTransferred; i++)
    {
        ...
        if (msgReceived)
        {
            ...
            case MAVLINK_MSG_ID_OPTICAL_FLOW:
            {
                ...
                terpcopter_comm::OpticalFlow optFlowMsg;
...
{
    terpcopter_comm::Mavlink mavlink;
